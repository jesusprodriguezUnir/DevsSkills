...
description: Crea un pull request con estructurada vinculada al contexto de la tarea en la hoja de ruta
arguments:
  - name: task_id
...
Crea una PR con una estructura descrita vinculada al contexto de la tarea en la hoja de ruta.
...
2. Comprobar confirma sin fusionar: `git log <tronco>..HEAD --oneline`. Salir si no hay ninguno.
3. Detectar plataforma: `command -v gh` (GitHub) o `command -v glab` (GitLab).
4. Verificar remoto: `git remoto get-url origin`.

### 2. Preparar compromisos

1. **Protección de tronco**: Si estás en el tronco (main/master/detectado), crea y cambia a una rama de funcionalidad (`git checkout -b feature/...` o `fix/...`).
2. **Comprobar cambios sin confirmar**: `git status --porcelain`. Si está limpio, salte al paso 3.
3. **Organizar commits atómicos**: Agrupar cambios en unidades lógicas. Confirmar en orden de dependencia (deps/config → tipos/interfaces → lógica central → funcionalidades dependientes). Siga el formato de `docs-eng-process/conventions.md`.
4. **Aplicar estrategia de pruebas**: Empaquetar pruebas con los commits de funcionalidad. Para correcciones de errores, confirme primero la prueba que falló, luego la corrección.
...
1. Obtener task_id de `$ARGUMENTS[task_id]` o extraer el nombre de la rama (regex `([Tt]-?\d+)`).
...
Usar `$ARGUMENTS[base]` si se proporciona; de lo contrario, seguir la detección de tronco en `docs-eng-process/agent-workflow.md` (Procedimiento de Ramas). Registrar el tronco detectado en el registro de ejecución.
...
- `docs/roadmap.md`: Añadir URL del PR a la entrada de la tarea
...
|-------|-------|----------|
| Sin comete pendientes | La rama está al día con el tronco | Informar al usuario de que no se necesita PR |
| Remoto no configurado | El control remoto de Git no está configurado | `git remoto agregar origen <url>` |
